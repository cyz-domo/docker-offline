...
function modernIndexHtml(env) {
  ...
  :root{font-family:Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",system-ui,sans-serif;color:#172033;background:#f4f7fb}*{box-sizing:border-box}body{margin:0;min-height:100vh;background:radial-gradient(circle at 10% 0,#dbeafe,transparent 35%),radial-gradient(circle at 95% 10%,#e0e7ff,transparent 28%),#f4f7fb}.shell{max-width:880px;margin:auto;padding:28px 20px 60px}.nav{display:flex;justify-content:space-between;align-items:center;margin-bottom:58px}.brand{display:flex;align-items:center;gap:10px;text-decoration:none;color:#172033;font-weight:750}.mark{display:grid;place-items:center;width:38px;height:38px;border-radius:12px;color:white;background:linear-gradient(135deg,#2563eb,#7c3aed);box-shadow:0 8px 20px #4f46e540}.nav a:last-child{color:#526079;text-decoration:none;font-size:14px;font-weight:650}.hero{margin-bottom:28px}.eyebrow{color:#2563eb;font-size:13px;font-weight:750;letter-spacing:.08em;text-transform:uppercase}.hero h1{font-size:clamp(34px,6vw,52px);line-height:1.08;letter-spacing:-.04em;margin:10px 0 14px}.hero p,.hint{color:#718096}.hero p{font-size:17px;margin:0}.card{background:#ffffffdf;border:1px solid white;border-radius:24px;padding:28px;box-shadow:0 18px 50px #1e3a8a14;backdrop-filter:blur(12px)}.grid{display:grid;grid-template-columns:1.1fr .9fr;gap:18px}.field label{display:block;color:#344054;font-size:13px;font-weight:750;margin-bottom:8px}.input,.select{width:100%;height:48px;border:1px solid #d8e0ed;border-radius:12px;background:white;color:#172033;padding:0 14px;font:inherit;outline:none}.input:focus,.select:focus{border-color:#6d8df7;box-shadow:0 0 0 4px #4f46e51a}.custom{margin-top:12px}.hint{font-size:12px;margin:7px 0 0}.toggle{display:flex;align-items:center;gap:9px;margin:22px 0;color:#475467;font-size:14px}.toggle input{width:18px;height:18px;accent-color:#4f46e5}.primary{width:100%;height:50px;border:0;border-radius:13px;background:linear-gradient(135deg,#2563eb,#4f46e5);color:white;font:700 15px inherit;cursor:pointer;box-shadow:0 10px 20px #4f46e533}.primary:disabled{opacity:.65;cursor:wait}.result{margin-top:20px;padding:20px;border-radius:16px;background:#f8fafc;border:1px solid #e5eaf2}.result-head{display:flex;justify-content:space-between;align-items:center;margin-bottom:14px}.status{border-radius:999px;padding:5px 10px;font-size:12px;font-weight:750}.building,.queued,.starting{color:#2563eb;background:#dbeafe}.success{color:#15803d;background:#dcfce7}.failed{color:#b42318;background:#fee4e2}.meta{display:grid;grid-template-columns:1fr 1fr;gap:10px;color:#718096;font-size:13px}.meta b{display:block;color:#172033;font-size:14px}.actions{display:flex;gap:9px;flex-wrap:wrap;margin-top:16px}.action{padding:9px 12px;border-radius:9px;background:#eef2ff;color:#3730a3;text-decoration:none;font-size:13px;font-weight:700}.download{background:#dcfce7;color:#166534}.error{color:#b42318;font-size:13px;margin-top:12px}.foot{text-align:center;color:#8290a8;font-size:12px;margin-top:20px}@media(max-width:640px){.shell{padding:20px 14px 44px}.nav{margin-bottom:40px}.card{padding:20px;border-radius:20px}.grid{grid-template-columns:1fr}.meta{grid-template-columns:1fr}}
  </style></head><body><div class="shell"><nav class="nav"><a class="brand" href="/"><span class="mark">D</span><span>Docker Offline Builder</span></a><a href="/history">构建历史　→</a></nav><section class="hero"><div class="eyebrow">Offline package</div><h1>构建你的 Docker 离线包</h1><p>选择版本与目标架构，提交后由 GitHub Actions 自动编译并生成下载链接。</p></section><main class="card"><form id="form"><div class="grid"><div class="field"><label for="preset">Docker 版本</label><select id="preset" class="select"><option value="29.6.1">29.6.1 · 推荐</option><option value="24.0.6">24.0.6</option><option value="23.0.6">23.0.6</option><option value="20.10.24">20.10.24</option><option value="19.03.15">19.03.15</option><option value="18.09.9">18.09.9</option><option value="custom">自定义版本</option></select><input class="input custom" name="version" value="29.6.1" pattern="\\d+\\.\\d+\\.\\d+(?:-[a-zA-Z0-9.]+)?" placeholder="例如 28.0.0" required><p class="hint">支持 X.Y.Z 或带后缀的版本号</p></div><div class="field"><label for="arch">目标架构</label><select id="arch" name="arch" class="select"><option value="x86_64">x86_64 · Intel / AMD</option><option value="aarch64">aarch64 · ARM64</option></select><p class="hint">请选择目标机器的 CPU 架构</p></div></div><label class="toggle"><input type="checkbox" name="chinaMirror"> 使用国内镜像加速下载</label>${widget}<button class="primary" id="submit" type="submit">开始构建　→</button></form><div id="result"></div></main><div class="foot">构建记录保留近 15 天 · GitHub Actions 提供编译资源</div></div><script>
  const form=document.querySelector('#form'),result=document.querySelector('#result'),submit=document.querySelector('#submit'),preset=document.querySelector('#preset'),version=document.querySelector('input[name="version"]');preset.onchange=()=>{if(preset.value==='custom'){version.value='';version.focus()}else version.value=preset.value};version.oninput=()=>{preset.value=[...preset.options].some(o=>o.value===version.value)?version.value:'custom'};const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const labels={starting:'准备中',queued:'排队中',building:'构建中',success:'构建成功',failed:'构建失败',expired:'已过期'};
  form.onsubmit=async e=>{e.preventDefault();submit.disabled=true;submit.textContent='正在提交…';const d=Object.fromEntries(new FormData(form));d.chinaMirror=!!d.chinaMirror;const token=document.querySelector('[name="cf-turnstile-response"]');if(token)d.turnstileToken=token.value;result.innerHTML='<div class="result">正在创建构建任务…</div>';const x=await fetch('/api/jobs',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(d)}),j=await x.json();if(!x.ok){result.innerHTML='<div class="result error">'+esc(j.error||'提交失败')+'</div>';submit.disabled=false;submit.textContent='开始构建　→';return}poll(j.job.id)};async function poll(id){const x=await fetch('/api/jobs/'+encodeURIComponent(id)),j=await x.json();if(!x.ok){result.innerHTML='<div class="result error">'+esc(j.error||'任务查询失败')+'</div>';submit.disabled=false;submit.textContent='开始构建　→';return}const s=j.status||'starting';result.innerHTML='<div class="result"><div class="result-head"><b>构建任务</b><span class="status '+esc(s)+'">'+esc(labels[s]||s)+'</span></div><div class="meta"><div>Docker 版本<b>'+esc(j.version)+'</b></div><div>目标架构<b>'+esc(j.arch)+'</b></div></div><div class="actions">'+(j.runUrl?'<a class="action" target="_blank" href="'+esc(j.runUrl)+'">查看 GitHub 构建 ↗</a>':'')+(j.downloadUrl?'<a class="action download" target="_blank" href="'+esc(j.downloadUrl)+'">下载离线包 ↓</a>':'')+'</div>'+(j.error?'<div class="error">'+esc(j.error)+'</div>':'')+'</div>';if(['success','failed','expired'].includes(s)){submit.disabled=false;submit.textContent='开始构建　→'}else setTimeout(()=>poll(id),5000)}
  </script></body></html>`;
}

function modernHistoryHtml() {
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Docker 构建历史</title><style>:root{font-family:Inter,-apple-system,BlinkMacSystemFont,"Segoe UI",system-ui,sans-serif;color:#172033;background:#f4f7fb}*{box-sizing:border-box}body{margin:0;min-height:100vh;background:radial-gradient(circle at 10% 0,#dbeafe,transparent 35%),#f4f7fb}.shell{max-width:900px;margin:auto;padding:28px 20px 60px}.nav{display:flex;justify-content:space-between;align-items:center;margin-bottom:58px}.brand{display:flex;align-items:center;gap:10px;text-decoration:none;color:#172033;font-weight:750}.mark{display:grid;place-items:center;width:38px;height:38px;border-radius:12px;color:white;background:linear-gradient(135deg,#2563eb,#7c3aed)}.nav a:last-child{color:#526079;text-decoration:none;font-size:14px;font-weight:650}.card{background:#ffffffdf;border:1px solid white;border-radius:24px;padding:28px;box-shadow:0 18px 50px #1e3a8a14}.eyebrow{color:#2563eb;font-size:13px;font-weight:750;letter-spacing:.08em;text-transform:uppercase}.title{font-size:clamp(32px,5vw,44px);line-height:1.1;letter-spacing:-.04em;margin:10px 0 8px}.muted,.time{color:#718096}.item{display:flex;align-items:center;justify-content:space-between;gap:18px;border-top:1px solid #e8edf5;padding:18px 0}.item:first-child{border-top:0}.name{font-weight:750}.time{font-size:13px;margin-top:4px}.actions{display:flex;gap:8px;flex-wrap:wrap}.action{padding:9px 12px;border-radius:9px;background:#eef2ff;color:#3730a3;text-decoration:none;font-size:13px;font-weight:700}.download{background:#dcfce7;color:#166534}@media(max-width:640px){.shell{padding:20px 14px 44px}.nav{margin-bottom:40px}.card{padding:20px;border-radius:20px}.item{align-items:flex-start;flex-direction:column;gap:10px;width:100%}}</style></head><body><div class="shell"><nav class="nav"><a class="brand" href="/"><span class="mark">D</span><span>Docker Offline Builder</span></a><a href="/">← 返回构建</a></nav><main class="card"><div class="eyebrow">Build archive</div><h1 class="title">构建历史</h1><p class="muted">近 15 天的构建任务和公开下载包</p><div id="list" class="muted">加载中…</div></main></div><script>const list=document.querySelector('#list');const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));fetch('/api/history').then(x=>x.json()).then(data=>{const items=[...(data.releases||[])].sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt));if(!items.length){list.textContent='暂无近 15 天构建记录';return}list.innerHTML=items.map(i=>'<div class="item"><div><div class="name">'+esc(i.name||'Docker 离线包')+'</div><div class="time">'+esc(new Date(i.createdAt).toLocaleString())+'</div></div><div class="actions">'+(i.url?'<a class="action" target="_blank" href="'+esc(i.url)+'">查看构建 ↗</a>':'')+(i.downloadUrl?'<a class="action download" target="_blank" href="'+esc(i.downloadUrl)+'">下载 ↓</a>':'')+'</div></div>').join('')}).catch(()=>{list.textContent='历史记录加载失败，请稍后重试'})</script></body></html>`;
}
